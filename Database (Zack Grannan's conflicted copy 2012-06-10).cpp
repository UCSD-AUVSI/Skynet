...
DatabaseConnection::reset(void)
{
	bool retVal = false;
	_command->Connection = _database;
	_command->CommandText = "TRUNCATE TABLE candidates;" + 
						    "TRUNCATE TABLE description;" + 
						    "TRUNCATE TABLE gps_position;" + 
						    "TRUNCATE TABLE location;" + 
						    "TRUNCATE TABLE telemetry;" + 
						    "TRUNCATE TABLE unverified_targets;" + 
							 "TRUNCATE TABLE verified_targets";	
	try
	{
...
DatabaseConnection::addCandidate( CandidateRowData ^ data)
{
	...
	if ( data->telemetry->telemetry_id == 0 ){
		if ( data->telemetry->planeLocation->gps_id == 0 ){
			gps_id = DatabaseConnection::addGPSPosition(data->telemetry->planeLocation);
		}else{
		...
		telemetry_id = result->GetInt32(0);
	}else{
...
DatabaseConnection::addUnverified( UnverifiedRowData ^ data)
{
	...
	if ( data->candidate->candidateid == 0 ){
		candidate_id = addCandidate(data->candidate);
	}else{
	...
	if (data->description->description_id == 0){
		...
		auto result = runQuery(query);
		description_id = result->GetInt32(0);
	}else{
...
DatabaseConnection::addVerified( VerifiedRowData ^ data)
{
	...
	if ( data->target->targetid == 0){
		targetid = DatabaseConnection::addUnverified(data->target);
	}else{
		targetid = data->target->targetid;
	}

	if (data->centerGPS->gps_id == 0){
		center_gps_id = DatabaseConnection::addGPSPosition(data->centerGPS);
	}else{
...
DatabaseConnection::modifyDescription( DescriptionRowData ^ data )
{
	...
	DatabaseConnection::runQuery(query);
}
...
DatabaseConnection::candidateWithID(String ^ id)
{
	...
	return (CandidateRowData ^)getAllRows(query,Candidate_Table)[0];
}
...
DatabaseConnection::unverifiedWithID(String ^ id)
{
	...
	return (UnverifiedRowData ^)getAllRows(query,Unverified_Table)[0];
}
...
DatabaseConnection::verifiedWithID(String ^ id)
{
	...
	return (VerifiedRowData ^)getAllRows(query,Target_Table)[0];
}
...
{
	array<Object ^>^ rows;
	String ^ query = CANDIDATE_QUERY_STRING;
...
{
	array<Object ^>^ rows;
	String ^ query = UNVERIFIED_QUERY_STRING;
...
{
	array<Object ^>^ rows;
	String ^ query = VERIFIED_QUERY_STRING;
...
VerifiedRowData ^ DatabaseConnection::verifiedFromReader(OdbcDataReader ^ theReader)
{
	...
	try {
		...
		row->verifiedid = theReader->GetInt32(col++);
		...
		row->centerGPS->lat = theReader->GetInt32(col++);
		row->centerGPS->lon = theReader->GetInt32(col++);
		row->centerGPS->alt = theReader->GetInt32(col++);
	}
