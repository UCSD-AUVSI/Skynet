...
void Comms::sendWifiAbortCommand()
{
	PRINT("Comms: send wifi abort");
	array<System::Byte> ^ passthrough = gcnew array<System::Byte>(1);
	passthrough[0] = 'C';
	autopilot->sendPassthroughPacket(passthrough);
}

void Comms::sendAutopilotWaypoints(array<Waypoint ^>^ waypoints)
{
	autopilot->sendWaypointsInThread(waypoints);
}

void Comms::sendZoom(unsigned __int32 zoom)
{
	PRINT("Comms: send " + zoom);
	array<System::Byte> ^ passthrough = gcnew array<System::Byte>(1);
	passthrough[0] = '0' + zoom;
	autopilot->sendPassthroughPacket(passthrough);
}
