...
bool AutopilotComport::connect(String ^ portName)
{	
	passthroughBuffer = nullptr;
	if(!__super::connect(portName)){
		return false;
	}
...
void AutopilotComport::writeDataInThread( Object ^ buffer)
{
	PRINT("write data" + buffer);
	thePort->writeEncodedData( (array<System::Byte> ^)buffer );
}

void AutopilotComport::writeData( array<System::Byte> ^ inBuffer )
{
	Thread ^ sendThread = gcnew Thread(gcnew ParameterizedThreadStart(this, &AutopilotComport::writeDataInThread));
	sendThread->Name = "Temp AutopilotComport Write Thread";
	sendThread->Start((Object ^)inBuffer);
}
...
UAVCommand ^ AutopilotComport::commandFromWaypoint(Waypoint^ waypoint)
{
	UAVCommand ^ command = gcnew UAVCommand();
	command->destAddr = 2418; // TODO: Use planeAdress
	command->commandType = 0;
	command->lat = waypoint->lat;
	command->lon = waypoint->lon;
	command->speed = waypoint->speed;
	command->altitude = waypoint->alt;
	command->gotoIndex = 0;
	command->time = 0;
	command->radius = 0;
	command->flareSpeed = 0;
	command->flareAltitude = 0;
	command->descentRate= 0;
	command->approachAltitude= 0;
	command->approachLongitude= 0;
	command->unusedOne= 0;
	command->unusedTwo= 0;
	return command;
}
...
void AutopilotComport::analyzeData( array<System::Byte> ^ inBuffer )
{
	...
	//	System::Diagnostics::Trace::WriteLine("AutopilotComport::receiveData() after:" + newPacket->Length + " buffer:" + bufferToString(newPacket));
		...
		if(newPacket != nullptr){  //if the decoded packet's xor worked out
			...
			if(kestrelPacketType == 5) // pass through from rabbit/ardupilot/wifi board
			{
				...
				array<System::Byte> ^ passthroughPacket = gcnew array<System::Byte>(dataSize);
				int packetIndex = 10;
				for (int i = 0; i < dataSize; i++)
					passthroughPacket[i] = newPacket[packetIndex++];

				receivePassthrough(passthroughPacket);
				//((RabbitComport ^)rabbit)->analyzeData(receivePassthrough);
			}
			...
			else if(kestrelPacketType == 248)  //navigation packet
			{
				...
				planeState->gpsAltitude = (double)(getUInt16FromBytes(newPacket, 7)) / 6.0 - 1000.0;
				planeState->gpsHeading = ((float)(getUInt16FromBytes(newPacket, 9)) / 1000.0f)*(float)(180.0/PI);
				planeState->gpsLatitude = (double)getFloatFromBytes(newPacket, 13);
				planeState->gpsLongitude = (double)getFloatFromBytes(newPacket, 19);
			...
			else if(kestrelPacketType == 249) //telemetry packet
			{
				...
				planeState->altitudeHAL = (double)(getUInt16FromBytes(newPacket, 5)) / 6.0 - 1000.0;
				planeState->velocity = (float)(getUInt16FromBytes(newPacket, 7)) / 20.0f - 10.0f;
				planeState->roll = (double)(getInt16FromBytes(newPacket, 9)) / 1000.0;
				planeState->pitch = (double)(getInt16FromBytes(newPacket, 11)) / 1000.0;
				planeState->heading = ((double)(getUInt16FromBytes(newPacket, 13)) / 1000.0);
...
void AutopilotComport::gotoLatLon(float lat, float lon) 
{
	...
	waypoint->alt = 100.0f;
	array<System::Byte> ^ packet = getVCPacket32(gcnew array<UAVCommand ^>{commandFromWaypoint(waypoint)});
...
void AutopilotComport::sendWaypoints(Object ^ waypointsObject)
{
	array<Waypoint ^>^ waypoints = (array<Waypoint ^>^)waypointsObject;
	array<array<System::Byte>^>^ packets = getVCPackets11(waypoints);
	for each (array<System::Byte>^ packet in packets){
		writeData(packet);
...
void AutopilotComport::sendWaypointsInThread(array<Waypoint ^>^ waypoints)
{
	Thread ^ sendThread = gcnew Thread(gcnew ParameterizedThreadStart(this, &AutopilotComport::sendWaypoints));
	sendThread->Name = "Waypoint Sending Thread";
	sendThread->Start(waypoints);
}
...
void AutopilotComport::sendPassthroughPacket( array<System::Byte> ^ inBuffer )
{
	...
	writeData( getVCPacket10(5, getCommPacket5Data(inBuffer)) );  //one liner - zing!
}

void AutopilotComport::receivePassthrough( array<System::Byte> ^ inBuffer )
{
	char bufferChar = inBuffer[0];
	if (passthroughBuffer == nullptr && 'Z' == bufferChar)
	{
		passthroughBuffer = "" + inBuffer[0];
	}
	else if (passthroughBuffer != nullptr && passthroughBuffer->Length == 1 
			&& bufferChar >= '0' && bufferChar <= '9')
	{
		// zoom succeeded!! 
		passthroughBuffer = nullptr;

		GimbalInfo ^ packet = gcnew GimbalInfo();
		packet->zoom = inBuffer[0] - '0';
		...
		theDelegate->receiveRabbitPacket(packet);
	}
	else
	{
		// print byte in GUI console
		theDelegate->printToConsole(passthroughBuffer + "" + bufferChar, Color::Blue);
...
array<array<System::Byte> ^>^ AutopilotComport::getVCPackets11(array<Waypoint ^>^ waypoints){
	...
	int dataSize = 18;  
	int destAddr = 2418;
	int kestrelPacketType = 50;
	int commandType = 2;
	char speed;
	short alt;
	float lat,lon;
	int payloadByte = 0;
	int numCommands = waypoints->Length;
	array<array<System::Byte> ^>^ packets = gcnew array<array<System::Byte>^>(numCommands);
	for (int i = 0; i < numCommands; i++){

		alt = (__int16)(waypoints[i]->alt * 10.0);
		speed = (unsigned char)(waypoints[i]->speed * 2);
		lat = waypoints[i]->lat;
		lon = waypoints[i]->lon;

		packets[i] = gcnew array<System::Byte>(26);
		auto packet = packets[i];

		char pack[26];
		(*(int *)&(pack[0])) = instruction;
		(*(int *)&(pack[4])) = dataSize;
		(*(short *)&(pack[8])) = destAddr;
		pack[10] = kestrelPacketType;
		pack[11] = commandType;
		pack[12] = i;
		pack[13] = numCommands;
		(*(short *)&(pack[14])) = alt;
 ...
 */
array<System::Byte> ^  AutopilotComport::getVCPacket32(array<UAVCommand^>^ commands) 
{
	const int instruction = 32;
	int commandSize = 59; // 14*4 + 2+1
	const int dataSize = commandSize*commands->Length;
	const int headerSize = 8;
	int offset = headerSize;
	array<System::Byte> ^ packet = gcnew array<System::Byte>(dataSize + headerSize);

	for (int i = 0; i < sizeof(int); i++){
		packet[i] = ((char*)&instruction)[i];
	}
	for (int i = 0; i < sizeof(int); i++){
		packet[i+sizeof(int)] = ((char*)&commandSize)[i];
	}
	for (int i = 0; i < commands->Length; i++){
		offset = 2 * sizeof(int) + commandSize * i;
		pin_ptr<unsigned short> destAddrPointer = &(commands[i]->destAddr);
		pin_ptr<unsigned char> commandTypePointer = &(commands[i]->commandType);
		pin_ptr<float> latPointer = &(commands[i]->lat);
		pin_ptr<float> lonPointer = &(commands[i]->lon);
		pin_ptr<float> speedPtr = &(commands[i]->speed);
		pin_ptr<float> altPointer = &(commands[i]->altitude);
		pin_ptr<unsigned short> gotoIndexPtr = &(commands[i]->gotoIndex);
		pin_ptr<unsigned short> timePtr = &(commands[i]->time);
		pin_ptr<float> radiusPtr = &(commands[i]->radius);
		pin_ptr<float> flareSpeedPtr = &(commands[i]->flareSpeed);
		pin_ptr<float> flareAltitudePtr = &(commands[i]->flareAltitude);
		pin_ptr<float> breakAltitudePtr = &(commands[i]->breakAltitude);
		pin_ptr<float> descentRatePtr = &(commands[i]->descentRate);
		pin_ptr<float> approachAltitudePtr = &(commands[i]->approachAltitude);
		pin_ptr<float> approachLongitudePtr = &(commands[i]->approachLongitude);
		pin_ptr<__int32> unusedOnePtr = &(commands[i]->unusedOne);
		pin_ptr<float> unusedTwoPtr = &(commands[i]->unusedTwo);

		
		char* ptr = (char*)destAddrPointer;
		for (int j = offset; j < offset + sizeof(__int16); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(short);

		ptr = (char*)commandTypePointer;
		for (int j = offset; j < offset + sizeof(char); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(char);

		ptr = (char*)latPointer;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
			PRINT("val lat " + ptr[j-offset]);
		}
		offset += sizeof(float);
		...
		ptr = (char*)flareSpeedPtr;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(float);

		ptr = (char*)flareAltitudePtr;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(float);

		ptr = (char*)breakAltitudePtr;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(float);

		ptr = (char*)descentRatePtr;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
		}
		offset+= sizeof(float);

		ptr = (char*)approachAltitudePtr;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(float);

		ptr = (char*)approachLongitudePtr;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(float);

		ptr = (char*)unusedOnePtr;
		for (int j = offset; j < offset + sizeof(__int32); j++){
			packet[j] = ptr[j-offset];
		}
		offset += sizeof(__int32);

		ptr = (char*)unusedTwoPtr;
		for (int j = offset; j < offset + sizeof(float); j++){
			packet[j] = ptr[j-offset];
		}
	}
