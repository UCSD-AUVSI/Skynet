...
Auvsi_Radon::squashPixel(float position, int line)
{
	...
	pixelValue[0] += leftWeight*value;// multiply by image pixel value to support non-binary images
	radon.at<VT>(line, (int)position - 1) = pixelValue;
	
	// right side
	pixelValue = radon.at<VT>(line, (int)position);
	pixelValue[0] += rightWeight*value;// multiply by image pixel value to support non-binary images
