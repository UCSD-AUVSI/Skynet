...
Auvsi_Fft::computeFFT(Complex *input, int length)
{
	...
	cv::Mat src = cv::Mat(length, 1, CV_32FC2, (void *)input); // convert input to cv::Mat matrix
	cv::Mat dst = cv::Mat(length, 1, CV_32FC2);				// allocate result
...
Auvsi_Fft::addToFeatureDescriptor(Complex *input, int lineNumber)
{
	...
	for (int i = NUMBER_COEFFICIENTS_TO_KEEP*lineNumber, j = 0; i < NUMBER_COEFFICIENTS_TO_KEEP*(lineNumber + 1); i++, j++) {
		x = input[j].x;
		y = input[j].y;
...
Auvsi_Fft::computeFeatureDescriptor()
{
	...
	if (!COMPUTE_ROW) {		// compute each column
		...
		for (int i = 0; i < inputImage.size().width; i++) {
			...
			// 	printf("%f, %f\t", currentColumn[j].x, currentColumn[j].y); // DEBUG
