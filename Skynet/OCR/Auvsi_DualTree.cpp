...
#define LENGTH_TO_PAD_TO 64 // (2^(J-1))*LENGTH_OF_FILTER;
...
Auvsi_DualTree::Auvsi_DualTree()
{
	...
	inputVector = new float[LENGTH_OF_VECTOR];
...
{
	// printf("input: %d ?= %d\n",inputImage.size().width, LENGTH_OF_VECTOR);
	if (isRow) {
		...
		for (int i = 0; i < inputImage.size().width; i++) {
			...
			// printf("%f=%f=%f\t", inputVector[i],ptrBegin[i],inputImage.at<float>(lineNumber,i));
		}
	} else {
...
Auvsi_DualTree::cshift(float * input, int length, int shift)
{
	...
	for (int i = 0, j = (length + shift)%length; i < length; i ++, j = (j+1)%length) {
		result[j] = input[i];
		// printf("%d->%d\t%f\n",i,j,input[i]); // DEBUG
	}
	return result;
 ...
 *		equiv to rundualtree+dualtree() in matlab script
 */
void
Auvsi_DualTree::computeLine()
{	
	//dualtree(x=inputVector, J, Faf, af) // J=4
...
Auvsi_DualTree::addToResults(int lineNumber)
{
	...
	// printf("\noutput: %d",pointer);
	for (int i = 0; i < 4; i++) {
		// printf("(%f,%f)\t", passFive[i].x, passFive[i].y);
...
void Auvsi_DualTree::setImage(cv::Mat image) 
{
