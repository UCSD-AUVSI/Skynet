...
PlaneWatcher::PlaneWatcher(Object ^ theParent, IntelligenceController ^ intelligenceController)
{ 
...
void PlaneWatcher::planeStateUpdated()
{
	...
	intelligenceController->queryCameraAndUpdateMapAndImage(); 
}
...
PlaneState ^ PlaneWatcher::predictLocationAtTime( float timeOffset )
{
	...
	//	lock l(this); 
		...
		if (smallestIndex == -1) 
		{
			...
			retval->gimbalInfo->pitch = gimbalDegreesToRaw(-retval->telemData->pitch); // 3000
			retval->gimbalInfo->roll = gimbalDegreesToRaw(-retval->telemData->roll); // 3000
