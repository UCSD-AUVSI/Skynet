...
//		Delegates::rowDataToVoid ^ saliencyAddTarget;
		...
		Delegates::uavCommandArraytoVoid ^ checkboxListDelegate;
...
private: System::Windows::Forms::Label^  label7;
	...
	public:
		Form1(void)
		{
			...
			thePlaneWatcher = gcnew Communications::PlaneWatcher(this,appController->getIntelligenceController());
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->mapCenterGPSLabel->Location = System::Drawing::Point(2859, 611);
			...
			this->mapMouseGPSLabel->Location = System::Drawing::Point(2740, 611);
			...
			this->gpsCheckboxList->Location = System::Drawing::Point(2309, 91);
			...
			this->waypointsLabel->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 14.25F, System::Drawing::FontStyle::Regular, System::Drawing::GraphicsUnit::Point, 
				static_cast<System::Byte>(0)));
			this->waypointsLabel->Location = System::Drawing::Point(2397, 45);
			...
			// sendWaypointsButton
			// 
			this->sendWaypointsButton->Location = System::Drawing::Point(2378, 531);
			...
			this->Controls->Add(this->mapMouseGPSLabel);
...
public: System::Void insertCandidateData( Database::CandidateRowData ^ data) 
		{
			...
			if ( !grid-> InvokeRequired )
			{
				...
				try
				{
					...
					// thumbnail = Image::FromFile( path ); DEBUG
...
public: System::Void insertUnverifiedData( Database::UnverifiedRowData ^ data) 
		{
			...
			if ( !grid-> InvokeRequired )
			{
				...
				double lat, lon, alt;
				Vision::GeoReference::getUnverifiedGPS(data, lat, lon, alt);
...
public: System::Void fillGpsCheckboxList ( array<UAVCommand ^>^ commands )
		{
			auto gpsCheckboxList = this->gpsCheckboxList;
			if ( gpsCheckboxList->InvokeRequired ){
				checkboxListDelegate = gcnew Delegates::uavCommandArraytoVoid(this,&Form1::fillGpsCheckboxList);
				this->Invoke(checkboxListDelegate,(Object ^)commands);
			}else{
				auto gpsListItems = this->gpsCheckboxList->Items;
				for ( int i = 0; i < min(commands->Length,17); i++){
					gpsListItems->Add(commands[i],true);
				}
				for ( int i = 17; i < commands->Length; i ++){
					gpsListItems->Add(commands[i],false);
				}
			}
		}
...
public: System::Void insertVerifiedTargetData( Database::VerifiedRowData ^ data) 
		{
			...
			{
				rowNum = grid->Rows->Add();
				grid->Rows[rowNum]->Cells[0]->Value = "" + data->verifiedid;
...
private: System::Void sendPassthroughPacketButton_Click(System::Object^ sender, System::EventArgs^  e) {
			String ^ packetText = passthroughPacketTextBox->Text;
			array<wchar_t>^ chars = packetText->ToCharArray();
			if ( chars->Length > 1024 ) return;
			array<System::Byte>^ packet = gcnew Array<Syste::Byte>(chars->Length);
			for (int i = 0; i < chars->Length; i++){
				packet[i]=(char)(chars[i]);
			}
			auto autopilot = this->appController->getSimHandler()->theTelSimulator->theAutopilotComport;
			autopilot->sendPassthroughPacket(packet);
...
private: System::Void sendWaypointsButton_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			auto waypointArray = Enumerable::ToArray(Enumerable::Cast<Waypoint ^>(gpsCheckboxList->CheckedItems));
			for each(auto waypoint in waypointArray){
				PRINT(waypoint);
			}
			auto autopilot = this->appController->getSimHandler()->theTelSimulator->theAutopilotComport;
			auto packets = autopilot->getVCPackets11(waypointArray);
			for (int i = 0; i < packets->Length; i++){
				autopilot->writeData(packet);
			}
		 }
};
}
