...
VoteCounter::VoteCounter(Skynet::SkynetController ^ del) 
{
	...
	return; // DEBUG: remove this if we want votes to get counted
...
{
	if (votes == nullptr || votes->votes == nullptr || votes->targetid < 0)
		return nullptr;
...
void VoteCounter::refreshUnverifiedTargetsTable()
{
	...
	{
		if (votes[i] != nullptr)
			if (nullptr != theDatabase->verifiedTargetForID("" + votes[i]->targetid))
				votes[i] = nullptr;
	}
