...
void Autosearch::startUpdaterThread(){
	Thread ^ updateMapThread = gcnew Thread(gcnew ThreadStart(this,&Autosearch::queryCameraAndUpdateMapAndImage));
	updateMapThread->Name = "Autosearch updating thread.";
...
void Autosearch::queryCameraAndUpdateMapAndImage()
{
	while(true){
		...
		{
			PRINT("PlaneWatcher does not exist");
			return;
		}
