...
#define MAX_IMAGES_PER_FILE 50 // num images

#define COUNTER_UPDATE_FREQ 50 // ms
#define PLANE_UPDATE_FREQ 1000 // ms
#define IMAGERY_UPDATE_FREQ 1000 // ms
#define IMAGERY_FILE_UPDATE_FREQ 5000 // ms
#define TARGETS_UPDATE_FREQ 15000 // ms
...
GEarthHandler::GEarthHandler(PlaneWatcher ^ tpw, OpenGLForm::COpenGL ^ togl)
{
	...
	lastFrame = 0;
	imageryRoot = "imagery_root_" + DateTime::Now.ToString("o")->Replace(":", "-");
	imageryRootData = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"+
		"<kml xmlns=\"http://www.opengis.net/kml/2.2\">\n"+
		"<Folder>\n"+
		"<name>AUVSI Imagery</name>\n"+
		"<visibility>1</visibility>\n"+
		"<open>1</open>\n"+
		"<description>AUVSI Real-Time Imagery</description>\n";
...
void GEarthHandler::writeMasterFile()
{
	...
	String ^ file = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"+
		...
		"<flyToView>0</flyToView>\n"+
		"<Link>\n"+
		"<href>imagery/"+imageryRoot+".kml</href>\n"+
		"<refreshMode>onInterval</refreshMode>\n"+
		"<refreshInterval>100000</refreshInterval>\n"+
		"</Link>\n"+
		"</NetworkLink>\n"+
...
void GEarthHandler::runLoop()
{
	...
	try{
		while(!shouldEnd)
		{
			...
			if (imageryFrameCount % MAX_IMAGES_PER_FILE == 0)
			{
				startNewImageryFile();
				writeImageryRoot();
				updateImageryFile();
				++imageryFrameCount; // so that this only happens once
			}
...
void GEarthHandler::updateImageryData(PlaneState ^ state)
{
	...
	String ^imageName = theController->saveCurrentFrameAsImage(GOOGLE_EARTH_KML_FILE_IMAGERY_ROOT + currentImageryKMLName + "\\");
	...
	currentImageryData += "<GroundOverlay>\n"+
					"\t<name>"+imageName+"</name>\n"+
					"\t<Icon>\n"+
					"\t\t<href>"+ currentImageryKMLName+"/"+imageName+"</href>\n"+
					"\t\t<viewBoundScale>1.0</viewBoundScale>\n"+
					"\t</Icon>\n"+
					"\t<altitudeMode>clampToGround</altitudeMode>\n"+
					"\t<gx:LatLonQuad>\n"+
					"\t\t<coordinates>\n"+
					""+TLLon+","+TLLat+" "+TRLon+","+TRLat+" "+BRLon+","+BRLat+" "+BLLon+","+BLLat+"\n"+//+TRLon+","+TRLat+" "+TLLon+","+TLLat+" "+BLLon+","+BLLat+" "+BRLon+","+BRLat+"\n"+
					"\t\t</coordinates>\n"+
					"\t</gx:LatLonQuad>\n"+
					"</GroundOverlay>\n";
}

void GEarthHandler::startNewImageryFile()
{
	static int fileNum = 0;

	// pick a unique path, set currentImageryKMLName
	currentImageryKMLName = "imagery_" + DateTime::Now.ToString("o")->Replace(":", "-")->Replace(".", "-");

	// make a folder with that name
	System::IO::Directory::CreateDirectory(GOOGLE_EARTH_KML_FILE_IMAGERY_ROOT + currentImageryKMLName);

	// add network link for currentImageryKMLName.kml to imageryRootData
	imageryRootData += 
		"<NetworkLink>\n"+
		"\t<name>Imagery " + (++fileNum) + "</name>\n"+
		"\t<visibility>1</visibility>\n"+
		"\t<open>0</open>\n"+
		"\t<description>Subset of images taken from plane.</description>\n"+
		"\t<refreshVisibility>0</refreshVisibility>\n"+
		"\t<flyToView>0</flyToView>\n"+
		"\t<Link>\n"+
		"\t\t<href>"+currentImageryKMLName+".kml</href>\n"+
		"\t\t<refreshMode>onInterval</refreshMode>\n"+
		"\t\t<refreshInterval>10000</refreshInterval>\n"+
		"\t</Link>\n"+
		"</NetworkLink>\n";

	currentImageryData = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"+
		"<kml xmlns=\"http://www.opengis.net/kml/2.2\" xmlns:gx=\"http://www.google.com/kml/ext/2.2\">\n"+
		"<Folder>\n";
}

void GEarthHandler::writeImageryRoot()
{
	StreamWriter^ outfile = gcnew StreamWriter(GOOGLE_EARTH_KML_FILE_IMAGERY_ROOT + imageryRoot + ".kml" );

	outfile->Write(imageryRootData);
...
{
	StreamWriter^ outfile = gcnew StreamWriter(GOOGLE_EARTH_KML_FILE_IMAGERY_ROOT + currentImageryKMLName + ".kml" );

	outfile->Write(currentImageryData);
	outfile->Write("</Folder>\n</kml>");
...
//	array<VotesOnCandidate ^>^ targetData = theDatabase->getAllVotes();
	...
	for each(VotesOnCandidate ^ votes in targetData)
	{
		...
		CandidateRowData ^ matchingCandidate = theDatabase->candidateWithID("" + votes->targetid);
...
{
	return Double(lon).ToString("######.###########")+
				","+Double(lat).ToString("######.###########")+
				","+Double(alt).ToString("F");;
}
